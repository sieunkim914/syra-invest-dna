...
const defaultAnalysis: AnalysisResult = {
  ...
  fitScore: 84,
  summary:
    '이 기업은 사용자의 성장 지향적이고 혁신 선호적인 성향과 잘 맞습니다. 다만 시장 기대감이 이미 가격에 반영되었을 가능성이 있어 즉시 전액 진입보다 분할 접근이 적합합니다.',
  fitReasons: ['AI 산업의 장기 성장성', '변동성을 견딜 수 있는 성향', '미래 기술 선호와 부합'],
  opportunities: ['AI 인프라 수요 확대', '데이터센터 투자 증가', '생태계 확장'],
  risks: ['고평가 부담', '단기 변동성', '실적 기대치 과열'],
  personalizedWarning:
    '위험회피가 매우 낮기 때문에 좋은 기업을 발견하면 지나치게 빠르게 확신할 수 있습니다.',
  actionGuide: '즉시 전액 매수보다 3회 분할 진입, 목표 비중 제한, 실적 발표 전후 변동성 확인을 권장합니다.',
};
...
export function Screen3Result() {
  ...
  const analysis = (location.state as { analysis?: AnalysisResult } | null)?.analysis || defaultAnalysis;
  const score = Math.max(0, Math.min(100, Math.round(analysis.fitScore)));
    ...
    <div className="min-h-screen bg-white px-5 py-6 pb-20">
      <div className="mb-8">
        ...
        >
          ...
          <br />
          <span className="text-black">당신에게 {decisionLabel[analysis.decision]} 대상입니다</span>
        </h1>
      ...
      <div className="bg-black text-white p-6 rounded-3xl mb-6">
        <div className="mb-3">
          <div style={{ fontSize: '48px', fontWeight: 700, lineHeight: '1' }}>
            {score} <span className="text-white/60">/ 100</span>
          </div>
        </div>
        <div className="text-sm mb-3 text-white/70">투자 적합도</div>
        <div className="w-full bg-white/20 rounded-full h-2 overflow-hidden">
          <div className="bg-[#FF6B4A] h-full rounded-full" style={{ width: `${score}%` }} />
        </div>
      </div>

      <div className="bg-[#F5F5F7] p-5 rounded-3xl mb-6">
        <p className="text-sm leading-relaxed text-black/80">{analysis.summary}</p>
      </div>
      ...
      <div className="space-y-3 mb-6">
        ...
        <div className="bg-[#FF6B4A] text-white p-5 rounded-3xl">
          ...
          <p className="text-sm leading-relaxed ml-11">{analysis.personalizedWarning}</p>
        </div>
        ...
        <div className="bg-[#F5F5F7] border-2 border-black/10 p-5 rounded-3xl">
          ...
          <p className="text-sm leading-relaxed text-black/80 ml-11">{analysis.actionGuide}</p>
        </div>
      </div>
